...
# st.title("∑ MathOCR - 截图公式识别工具")
st.markdown("## **∑ MathOCR - 截图公式识别工具**")
...
if st.session_state.editor_area:
    ...
    with col1:
        ...
        # 这里的逻辑和之前一样：用 on_change 回调或者直接绑定 session_state
        # 这次我们用最简单的 key 绑定法
        
        text_input = st.text_area(
            label="源码",
            height=280,
            key="editor_area", # 绑定 key，Streamlit 会自动更新 session_state.editor_area
            help="在这里修改识别错误的字符，右侧会自动更新"
        )
        
        # 将 text_area 的最新值同步回我们的主变量 (其实可以直接用 session_state.editor_area)
        st.session_state.editor_content = text_input

    with col2:
        st.subheader("🏞️当前图片")
        st.image(image, caption="当前图片", width='stretch')
